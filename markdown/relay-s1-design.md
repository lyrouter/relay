...
| **LOG** | LOG-1 双模式编辑 3 · LOG-2 渲染 2 · LOG-3 工单卡片内联 1 · LOG-4 版本与回滚 3 · **LOG-5 附件 1.5**（S-25：适配器盲写，多一份容器契约测试）· LOG-6 分享 1.5 · LOG-7 模板 1 · LOG-8 搜索 2 · LOG-9 知识库标记 0.5 | **15.5** | FE / BE |
...
> 上表 62 pd 含 TA-1、通知与 WEB；正文口径的 **≈55.5 pd** 指四个主模块 + API（7+8.5+15.5+13+7.5 = 51.5）加 INT 4。三个数都列出来，避免评审时对不上。
>
...
>
> ⚠️ **61.5 → 62 是 S-25 的对价**：MinIO 适配器改为**不等真实实例、按标准 S3 盲写**，
> 代价是多买一份**容器化契约测试 + 一个往返冒烟脚本**（+0.5 pd，计入 LOG-5）。买它的理由是
> 时点：不买，适配器第一次见到真实 MinIO 就是上线前那一晚；买了，盲区被压缩到只剩部署形态
> （预签 host / 寻址风格 / 时钟 / bucket 策略），而那四样在部署清单上是可逐条确认的。

**与原 MVP 的差异**：MVP 68.5 pd → S1 62 pd，净 −6.5。

| 方向 | 明细 | 小计 |
|---|---|---:|
| 减 | BOT 10 · TA-2…TA-4 4 · INT 中随 BOT / Jira 停用推后的 4 · AC-6 + AC-7 2.5 · MT-5 1 | **−21.5** |
| 增 | API 7.5 · **WEB 4** · 通知 NT 1.5 · AC-9 租户归属 1 · INT-11 备份恢复演练 0.5 · **S-25 盲写适配器的容器契约测试 0.5** | **+15** |
...
- **载体的实现路径（✅ 已决策 S-25）**：**不等真实实例**，适配器按**标准 S3 语义**盲写（S3 兼容客户端 + MinIO 的 path-style 寻址），载体在装配处按配置切换，`FilesystemBlobStore` 留给开发与测试（key 布局两者完全一致，换载体**不搬对象、不改任何已存的 `blob_key`**）。盲写的验证手段是**容器化契约测试**（起一个真实 `minio/minio` 跑同一套 blob 契约）加一个**往返冒烟脚本**，真实实例上的偏差按 **BUG** 处理。
  > ⚠️ **盲写能写对语义，写不对部署形态。** 四处盲区写进[部署文档 §O-5](relay-s1-deploy.md#o-5-minio)，因为它们只能在部署时确认：① **预签名 URL 的 host**（内网 endpoint + 反代域名时，签出来的链接浏览器访问不到——症状是图片全裂而应用日志里什么都没有，因为浏览器直连对象存储）；② **path-style 寻址**；③ **时钟偏移**（预签有效期只有 5 分钟）；④ **bucket 必须私有**——匿名可读会让 S-11 那套「先鉴权、再签链接」整套白做。
  >
  > 另外两处是语义，在适配器里对齐：**大小限制要保持流式判定**（否则 25 MiB 限制退化成"先收完再拒绝"），以及 **`/blobs/{key}` 这条路由随载体消失**（它依赖文件系统独有的 `verify`/`open`，MinIO 下要么下线要么显式改为代理，不能留一个调用即报错的入口）。
...
| **范围** | S1 范围与 **62 pd** 的量级按本文执行（原 57.5 + S-24 的 WEB 4 + S-25 的 0.5） | §1.1 · §3 |
