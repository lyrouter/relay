...
| | |
|---|---|
| **文档状态** | **已确认 · 可开工**。§12 的全部待澄清问题**已按建议决策**（含 RLS 下沉、自助注册租户归属、编号格式、API 契约三字段），决策记录见 [§12.1](#121-决策记录全部采纳建议)。**F-1…F-4 与 R-1…R-3 全部落地**（通知只做站内信 · pgroonga 可装 · 首个 API 消费方 = 网关 WebUI 反馈入口 · 自建 MinIO · 运维备份与离职停用由 WANGLI 认领 · BOT 排期第 7 周给出）。**F-5 也已落地**（事务性发信通道存在 ⇒ AC-1 邮箱验证原样实现）。**只剩 [F-6](#f-6-反馈链路的三个实现细节)** 反馈链路的三个产品细节（第 5 周前，不阻塞开工）→ [§12.2](#122-遗留项) |
| **范围（做）** | **MT** 多租户数据模型 · **AC** 账号（**自助注册为主**）· **LOG** 日志与知识撰写 · **TKT** 工单与看板 + **API** 工单对外 API |
| **范围（只留接口）** | TA 遥测适配器 · BOT 企微机器人 · ⏸ GH GitHub 同步 · ⏸ RAG/SEED 知识问答 |
| **工作量** | **≈ 55 pd ≈ 11 人·周**（§3）；按 1.7 人·周/日历周约 **6.5 个日历周** |
...
| 5–6 | LOG 收尾 · TKT 收尾 · 通知 · API-1/2/3 |
| 7 | API-4 webhook · API-5 OpenAPI 与契约测试 · INT 端到端 · 双轨试用启动 |
...
| 工单创建 / 编辑 / 流转 | ✅ | ✅ | ✕ |
| 评论 | ✅ | ✅ | ✕ |
| 查看内容 | 按分享级别 | 按分享级别 | **仅 L1 显式授权 + L3**（已决策 S-6：**不因加入空间而获得 L2**） |
...
| **范围** | S1 范围与 **57.5 pd** 的量级按本文执行 | §1.1 · §3 |
...
#### F-6 反馈链路的三个实现细节
§8.8 的链路已定，剩三个产品决策，都不阻塞 API 开工，但决定这条链路成不成立：

| # | 问题 | 建议 |
|---|---|---|
| ① | WebUI 是否展示处理进度？ | **展示**。轮询 `GET /tickets/{key}`，只显示状态与最后更新时间，**不显示内部评论**——内部讨论不该外泄给提交者 |
| ② | 工单关闭时是否通知提交者？ | **由 WebUI 侧通知**，Relay 只把状态变化告知 WebUI（轮询或 webhook）。**Relay 自己不直接触达网关用户**——那会让 Relay 变成对外系统，属 Phase 4 的范围与安全评审 |
| ③ | 反馈的默认 type 与优先级？ | **type=Bug，priority=P2**，由处理人分诊时调整。**不让提交者选优先级**——外部提交的优先级一律不可信，开放选择的结果是一周内全是 P0 |
...
| §8 API | **未覆盖** | **未覆盖** | **新增 API-1…API-5** |
| §9 通知 | §4.8④（企微部分推后） | §7.5 | **新增 NT-1 / NT-2** |
| §10 TA 接口位 | §4.2 | §3 | TA-1（TA-2…TA-4 推后） |
| §11 验收 | §4.11① | §8.1 | Milestone A exit criteria 的子集 |

**建议同步更新 TODO.md**：新增 API / NT / AC-9 三组任务，并把 BOT、TA-2…TA-4、AC-6、AC-7、MT-5 标为 S1 之后。本文未直接改 TODO.md——**等这份设计确认后再一次性改，避免任务清单在评审中途反复变动。**
