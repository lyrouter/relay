...
| 8 | [MinIO 实例信息](#o-5-minio) | 运维 | 附件适配器写不了（当前用文件系统载体） |
...
5 分钟链接），**但 MinIO/S3 适配器还没写**——故意没盲写：没有真实 MinIO 可测的适配器，
比一个明确缺失的适配器更糟。当前载体是文件系统（`FilesystemBlobStore`），**key 布局
与 MinIO 版本完全一致**，所以换载体不需要搬任何对象、不改任何已存的 `blob_key`。

需要运维给四样东西：**实例地址 · access key / secret · bucket 名 · 备份口径**。
给了就补适配器（约 0.5 pd）。

> 换成 MinIO 之后，`GET /blobs/{key}` 这条路由就不再有人访问：签名链接会直接指向对象
> 存储，浏览器不再回到应用。它现在存在只是因为当前载体是文件系统。
